...
fn server_command(data_dir: &Path) -> Command {
    ...
    command
        ...
        .env_remove("STRATUM_SAML_PROVIDER_KEY")
        .env_remove("STRATUM_SAML_IDP_ENTITY_ID_HASH")
        .env_remove("STRATUM_SAML_SP_ENTITY_ID_HASH")
...
#[test]
fn server_startup_with_partial_saml_config_creates_no_local_vfs_files() {
    let data_dir = TempDataDir::new("partial-saml-hosted-auth");
    let output = server_command(data_dir.path())
        .env("STRATUM_SAML_PROVIDER_KEY", RAW_SAML_PROVIDER_KEY)
        .output()
        .expect("stratum-server should execute");
    ...
    assert!(text.contains("STRATUM_HOSTED_AUTH_PROVIDER"));
    assert!(text.contains("STRATUM_HOSTED_AUTH_ENABLE_DEV"));
    assert!(text.contains("STRATUM_SAML_IDP_ENTITY_ID_HASH"));
    assert!(text.contains("STRATUM_SAML_SP_ENTITY_ID_HASH"));
