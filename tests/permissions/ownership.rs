...
fn only_root_can_chown_uid() {
    ...
    let result = try_run("chown bob: engineering/design.md", &mut fs, &mut alice);
...
fn root_can_chown() {
    ...
    run(
        "chown bob:engineering engineering/design.md",
        &mut fs,
        &mut root,
    );
    ...
    assert!(
        stat.contains("Uid: 2"),
        "uid should be bob's (2). stat: {stat}"
    );
}
...
fn group_read_access() {
    ...
    run(
        "write engineering/secret-eng.md Engineering secrets",
        &mut fs,
        &mut root,
    );
    run(
        "chown root:engineering engineering/secret-eng.md",
        &mut fs,
        &mut root,
    );
...
fn non_group_cannot_read_group_file() {
    ...
    run(
        "write engineering/eng-only.md For engineers only",
        &mut fs,
        &mut root,
    );
    run(
        "chown root:engineering engineering/eng-only.md",
        &mut fs,
        &mut root,
    );
    ...
    assert!(
        result.is_err(),
        "carol not in engineering, should not read 640 file"
    );
}
