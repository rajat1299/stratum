...
stratum has a built-in multi-user system with Unix-style permissions. This guide covers creating users, managing groups, setting permissions, and controlling access.

## Default State

A fresh stratum instance has one user and two groups:
...
```
alice@stratum:/ $ addagent deploy-bot
Created agent: deploy-bot (uid=3)
Token: a1b2c3d4e5f6...  (save this — shown only once)
```

The token is used for HTTP API authentication via `Authorization: Bearer <token>`. The raw token is shown once — stratum only stores its SHA-256 hash internally.
...
```
root@stratum:/ $ su root
root@stratum:/ $ deluser bob
Deleted user: bob
```
...
```
alice@stratum:/ $ addgroup engineering
...
```
alice@stratum:/ $ usermod -aG engineering bob
...
```
alice@stratum:/ $ usermod -rG engineering bob
...
```
root@stratum:/ $ delgroup engineering
...
```
alice@stratum:/ $ groups
alice wheel

alice@stratum:/ $ groups bob
bob engineering
```
...
stratum uses standard Unix-style permission bits: **owner**, **group**, and **other**, each with read (r), write (w), and execute (x).
...
```
alice@stratum:/ $ chmod 700 private/
alice@stratum:/ $ chmod 640 secret.md
```
...
root@stratum:/ $ chown bob readme.md

# Change owner and group
root@stratum:/ $ chown bob:engineering readme.md

# Change group only (owner can do this if they're in the target group)
alice@stratum:/ $ chown alice:engineering project/
```
...
```
alice@stratum:/ $ mkdir shared
alice@stratum:/ $ chmod 1777 shared
```
...
```
alice@stratum:/ $ mkdir team-docs
alice@stratum:/ $ chown alice:engineering team-docs
alice@stratum:/ $ chmod 2775 team-docs

# Bob creates a file — it gets group "engineering" automatically
bob@stratum:/ $ touch team-docs/notes.md
bob@stratum:/ $ ls -l team-docs/
...
```
deploy-bot@stratum:/ $ delegate bob
Delegating as bob

deploy-bot@stratum:/ $ touch docs/deploy-notes.md
# File is owned by bob, and both deploy-bot AND bob must have permission

deploy-bot@stratum:/ $ undelegate
Delegation cleared
```
...
```
deploy-bot@stratum:/ $ delegate :engineering
```
...
alice@stratum:~ $ adduser carol
alice@stratum:~ $ addagent ci-bot

# Create a shared team group
alice@stratum:~ $ addgroup dev-team
alice@stratum:~ $ usermod -aG dev-team alice
alice@stratum:~ $ usermod -aG dev-team bob
alice@stratum:~ $ usermod -aG dev-team carol

# Switch to root to create shared top-level directories
alice@stratum:~ $ su root

# Create a shared workspace with setgid
root@stratum:~ $ mkdir /project
root@stratum:~ $ chown alice:dev-team /project
root@stratum:~ $ chmod 2775 /project

# Create a drop-box directory (sticky bit — anyone can add, only owners can delete)
root@stratum:~ $ mkdir /submissions
root@stratum:~ $ chmod 1777 /submissions

# Switch back to alice
root@stratum:~ $ su alice

# Alice can also create private dirs in her home
alice@stratum:~ $ mkdir private
alice@stratum:~ $ chmod 700 private

# Bob creates a file in the shared project — inherits dev-team group
alice@stratum:~ $ su bob
bob@stratum:~ $ touch /project/design.md
bob@stratum:~ $ write /project/design.md # System Design Draft
bob@stratum:~ $ ls -l /project/
