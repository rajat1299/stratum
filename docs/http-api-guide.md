...
This is not the full durable filesystem/VCS cutover. Core filesystem state, VCS object bytes, commit vectors, and refs still live in the local `StratumDb` snapshot at `.vfs/state.bin`. The backend now has an executable durable core transaction semantics contract for object-byte promotion, object metadata, commit metadata, ref compare-and-swap, workspace-head update, audit append, and idempotency completion, plus an internal durable `CoreDb` runtime shape that can execute two metadata-only ref-management methods over composed stores. Object bytes, commit creation, ref listing, audit/idempotency/workspace-head transaction completion, and live Postgres/R2 HTTP route execution remain future work.
