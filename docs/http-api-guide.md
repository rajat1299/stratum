...
The same optional feature also exposes a Postgres-backed `IdempotencyStore` over the `idempotency_records` table (live contract tests reuse the existing migration-only schema without changing migrations). Rows store only hashed idempotency keys (`key_hash`), not raw `Idempotency-Key` header values; the adapter remains unhooked from `stratum-server` request handling.

Workspace-token issuance still rejects idempotency keys because replay persistence for secret-bearing responses is intentionally out of scope for this slice. Records have no expiration in the current migration until a retention model exists for durable runtime cutover.
