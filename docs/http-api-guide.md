...
In durable-cloud mode, `commits`, `inodes`, and `objects` are `null` because the local core database is intentionally not opened. The readiness block switches `local_core_required` and `local_core_available` to `false`, while durable object-store and recovery-store booleans are `true` after startup has opened and checked the hosted stores.
