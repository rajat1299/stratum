...
This is not the full durable filesystem/VCS cutover. Core filesystem state and all non-guarded VCS behavior still live in the local `StratumDb` snapshot at `.vfs/state.bin`. The guarded commit route is a narrow bridge that persists one `POST /vcs/commit` execution path through durable stores; durable auth/session, durable filesystem reads/writes, durable VCS log/status/diff/revert/ref-list routes, persisted recovery workers, distributed locking, and broad Postgres/R2 HTTP route execution remain future work.
