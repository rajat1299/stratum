...
The `sources` group describes the workspace projection rather than concrete storage values. `sources.workspace` reports the public mount root (`/`), the current backing store family (`local-state` or `durable-core`), and the identity context a mounted workspace needs. `sources.mounts` advertises the supported HTTP workspace API and process-local TypeScript SDK mount surfaces. `sources.provider_mounts` remains unavailable with an empty supported-kind list until remote/blob/provider mounts have durable source guarantees; the manifest does not expose provider endpoints, local paths, object keys, or repo ids.
...
- `POST /auth/saml/login` with JSON `{ "provider": "...", "saml_response": "...", "relay_state": "...", "org_id": "...", "repo_id": "..." }`. `relay_state` is optional. The default verifier denies SAML login. Tests can inject a provider-free verifier; normal operation does not parse production XML, fetch IdP metadata, validate live XML signatures, call provider networks, or store raw assertions.
...
SAML login is tenant-scoped before token issuance. The verified assertion org/repo must match the requested `org_id` and `repo_id`, the repo must be bound to that org through the tenant resolver, and the external identity/principal binding must already be known. Tenant mismatch, repo mismatch, unknown external identities, malformed metadata, unsupported bindings, expired or not-yet-valid assertions, ACS/audience/entity mismatch, assertion replay, disabled providers, and required-group mismatch fail closed without falling back to local/root identity. The provider-free SAML foundation validates bounded metadata/assertion fields and group mapping outputs supplied by the verifier seam; production XML signature validation, IdP metadata fetching, SCIM, group provisioning, invitations, and hosted admin UI remain out of scope.
...
| Header | Description |
|---|---|
| `X-Stratum-Org: <org-id>` | Resolve the hosted tenant before repo/workspace lookup |
| `X-Stratum-Workspace: <uuid>` | Resolve a bearer token as a hosted workspace token issued by the gateway |
| `X-Stratum-Repo: <repo-id>` | Select the hosted repo for admin durable routes |
...
### Sparse VFS Cache, Hydration Scheduler, Mount Adapter, And Daemon UX Status
...
The mount daemon UX foundation adds a local, provider-free control model for PID/socket/log metadata, redacted daemon status, bounded sanitized logs, and idempotent unmount outcomes. `stratumctl mount status`, `stratumctl mount logs`, and `stratumctl mount unmount` inspect that local model and render text or JSON without exposing raw tags, runtime paths, provider errors, object ids, SQL, bearer tokens, or backend credentials.

This does not change HTTP API behavior, route availability, durable-cloud startup gates, local `.vfs/state.bin` persistence, committed-read source selection, recovery/idempotency/audit semantics, or durable-cloud unsupported route output. `stratum-mount` remains snapshot-only over `db.snapshot_fs()`, and direct MCP/FUSE/REPL callers still fail closed under durable-cloud. Real sparse FUSE serving, NFS-over-localhost daemon lifecycle, macOS mount lifecycle, mount process management, write-back, and durable sparse mount enablement remain future work.
...
Audit event export is disabled by default. `STRATUM_AUDIT_EVENT_EXPORT_PROVIDER` defaults to `disabled`; this slice accepts only `provider-free-dev` with `STRATUM_AUDIT_EVENT_EXPORT_ENABLE_DEV=1`. Optional provider-free settings are `STRATUM_AUDIT_EVENT_EXPORT_MAX_ATTEMPTS` and `STRATUM_AUDIT_EVENT_EXPORT_MANDATORY_CLASSES`. Malformed or partial audit export configuration fails closed with env-name-only errors before local `.vfs` files are created. Real NATS, Kafka, Kinesis, broker URLs, cloud credentials, and production event-bus deployment are not configured by this foundation.
...
Migration 15 adds the conservative org/tenant foundation for durable metadata. It creates `organizations`, `org_memberships`, and `org_service_accounts`; adds org ids to hosted workspace, workspace-token, and workspace-principal rows; and adds `repos.org_id` with a non-destructive backfill to `default_org` for existing repos. Existing non-null repo org values are preserved, `default_org` is inserted for compatibility, and future repo inserts keep the default. This slice does not change `repos.id` or downstream `repo_id` foreign keys, so repo ids remain globally unique until a later org-local repo slug migration.
...
The optional `postgres` feature also includes a Postgres-backed `WorkspaceMetadataStore` over `workspaces`, `workspace_tokens`, and workspace principal rows. It stores optional org/repo identity for backwards-compatible local rows, preserves base/session refs and head-version updates, and persists only workspace-token secret hashes with normalized read/write prefixes. Hosted durable workspace bearer validation requires matching workspace, token, principal, org, and repo identity before constructing a mounted session. `STRATUM_BACKEND=durable` uses this store for hosted workspace endpoints. Workspace-token issuance can be idempotent only when secret replay KMS is configured; token rotation, expiry, revocation, and hosted secret-management behavior remain future work.

The optional `postgres` feature also includes a Postgres-backed `ReviewStore` over protected ref rules, protected path rules, change requests, approvals, reviewer assignments, and review comments. `STRATUM_BACKEND=durable` uses this store for review/protected-change endpoints. Review and policy rows remain repo-keyed in this slice, and route-level tenant resolution denies cross-org repo selection before those repo-scoped stores are used. Change-request base/head commit IDs remain lowercase SHA-256-shaped strings and are not foreign-keyed to the durable Postgres commit catalog. Under the guarded durable commit route, review creation and merge can still resolve durable refs and durable commit metadata for the selected repo; otherwise review routes preserve the existing local `.vfs/state.bin` behavior.
...
Audit export, when explicitly enabled in provider-free development mode, is a secondary export path. Local or Postgres audit persistence remains the durable system of record. The export wrapper publishes only the server-assigned `AuditEvent` after primary persistence succeeds, records bounded delivery attempts/status/lag metrics in process, and redacts export payload details before handing them to the provider-free sink. Best-effort export failures do not change route behavior; fail-closed behavior applies only to explicitly configured mandatory audit classes. Durable-cloud `/audit` listing remains unsupported, and production event-bus adapters are future work. See `docs/audit-posture.md` for the current audit posture note.
