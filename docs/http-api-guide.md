...
An internal durable `CoreDb` implementation path now exists for the future server core. It owns the composed backend store bundle and repo id, and binds its durable write ordering to the executable core transaction semantics contract. It is not a live route executor: every route-facing filesystem, search, tree, auth, and VCS method returns a stable `NotSupported` error without echoing request paths, usernames, tokens, ref names, request bodies, raw environment values, or durable runtime selector values. `STRATUM_CORE_RUNTIME=durable-cloud` still fails closed during startup before serving.
