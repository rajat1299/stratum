...
- Postgres FTS semantic search MVP shipped for durable-cloud (`GET /search/semantic`, migration 0019, SDK `search.semantic`); Slice 21 adds ACL snapshot filtering (migration 0020, `posix-tree-v1` snapshots, session-scoped pre-filter plus final recheck); vector/pgvector index remains future work
...
- Added tenant-scoped SCIM clients, users, groups, and group memberships. SCIM users bind external identities to existing tenant principals; SCIM groups map external groups to local gids; memberships track active principals. User/group creates and membership add/remove/deactivate retries converge on the represented tenant/client identity. User PATCH/DELETE binds the URL resource id to the body external id, and user/member mutations reject unknown tenant principals before mutating state.
...
- Added Postgres migration 0018 (`scim_provisioning_foundation`) with `scim_clients`, `scim_users`, `scim_groups`, and `scim_group_members`. The schema is additive, stores hashes/HMAC hashes and bounded fields only, enforces org/repo/client/principal shape, and includes strict adoption checks for SCIM table, index, key, hash, default, lifecycle, raw-material column, and constraint shape.
...
- The chunk cache validates stored byte length against actual bytes, chunk size, and chunk offset. SQLite foreign keys are enabled, schema/config version is checked on open, file-backed caches request WAL mode, public cache model `Debug` output redacts payloads/paths/cache identities, and tests require no Postgres, R2, durable-cloud env, or network.
...
- Spec/correctness review: no blocking findings. It confirmed no HTTP, committed-read, durable runtime, MCP, REPL, or mount wiring changed; `stratum-mount` remains snapshot-only through `db.snapshot_fs()`.
- Code-quality/security review: no blocking findings. Hardening feedback was fixed by capping lookup increments before SQLite integer overflow, enforcing chunk size/offset invariants, validating schema/config version on open, adding domain `CHECK` constraints, and redacting public cache model `Debug` output.
- `cargo fmt --all -- --check`
- `git diff --check`
- `cargo test --locked sparse_cache::tests --lib -- --nocapture` passed **16** tests
- `cargo check --locked -p stratum-core`
- `cargo test --locked -p stratum-core` passed **6** tests
- `cargo check --locked`
- `cargo check --locked --features postgres`
...
- `cargo clippy --locked --all-targets -- -D warnings`
- `cargo clippy --locked --all-targets --features postgres -- -D warnings`
- `cargo test --locked --lib --tests` passed, including **975** lib tests, **9** `stratum_mcp` tests, **5** `stratumctl` tests, **142** integration tests, **37** perf tests, **1** perf-comparison test, **72** permission tests, and **22** server-startup tests
- `cargo audit --deny warnings` passed after scanning **422** crate dependencies
...
- `STRATUM_CORE_RUNTIME` still defaults to `local-state`; local state remains the default runtime.
- `STRATUM_CORE_RUNTIME=durable-cloud` no longer requires `STRATUM_DURABLE_CORE_RUNTIME_ENABLE_DEV`.
- Durable-cloud startup still requires `STRATUM_BACKEND=durable`, auth/session readiness, policy readiness, repo-routing readiness, recovery readiness, a non-local repo id, idempotency retention/quota config, explicit Postgres posture, and explicit R2 posture.
- Missing or incomplete durable-cloud config continues to fail closed before local `.vfs/state.bin` creation.
...
Delivered from `docs/plans/2026-05-16-durable-cloud-vcs-review-mutations.md`.

Completed scope:

- Durable-cloud now serves VCS ref create/update, commit, and revert through durable stores with idempotency, policy/audit handling, and no local `.vfs/state.bin` fallback.
- Durable-cloud now serves protected ref/path rule routes and change-request mutations, including approvals, reviewers, comments, approval dismissal, reject, and merge, through durable review/protection stores.
- Durable-cloud admin mutations use the repo-scoped workspace bearer admin-principal seam: the bearer token must resolve to the matching repo/workspace and an active durable root or wheel principal. Local `Authorization: User root` remains unsupported in durable-cloud.
- `GET /v1/capabilities` revision `2026-05-16-2` advertises the durable-cloud VCS/review/protected mutation surfaces, their idempotency support, and the workspace-bearer/durable-admin-principal requirements.
...
- `GET /v1/capabilities` is an unauthenticated, cacheable manifest endpoint with revision `2026-05-16-2`.
...
Grounding: `.github/workflows/rust-ci.yml`, `scripts/ci-live-postgres-gate.sh`, `scripts/ci-live-r2-gate.sh`, `scripts/check-postgres-migrations.sh`, `scripts/check-r2-object-store.sh`, `docs/http-api-guide.md`, `docs/plans/2026-05-15-live-gates-in-ci.md`.
...
- Durable startup now fails closed for remote Postgres URLs without `sslmode=require`; explicit localhost, loopback hostaddr, and Unix-socket targets remain allowed without TLS for local development.
...
- No production KMS/secrets manager posture, distributed locks, or cross-store transaction boundary.
...
- At this slice's landing time, `durable`, `durable-cloud`, and `postgres-r2` were recognized as the future durable core runtime and `stratum-server` rejected them fail-closed before durable backend prerequisite validation, before migration preflight, before local state was opened, and before serving. Later durable-cloud slices added the current gated durable-cloud router.
...
The durable commit transaction executor skeleton created the first internal commit-execution contract behind the durable `CoreDb` runtime while broad durable HTTP serving was still fail-closed.
...
- At this slice's landing time, durable startup remained fail-closed for `STRATUM_CORE_RUNTIME=durable-cloud` before local state, durable backend validation, migration preflight, or serving. The later Broad Durable Core Runtime Incremental Enablement slice added the initial gated read-only durable-cloud router.
...
The durable commit transaction metadata preflight added the first internal read-only commit executor step behind the durable `CoreDb` runtime while broad durable HTTP serving was still fail-closed.
...
- At this slice's landing time, durable startup remained fail-closed for `STRATUM_CORE_RUNTIME=durable-cloud` before local state, durable backend validation, migration preflight, or serving.
...
The durable create-ref executor path completed the second narrow ref-management method inside the internal durable `CoreDb` implementation while broad durable HTTP serving was still fail-closed.
...
The durable update-ref executor path was the first narrow commit-oriented method that could execute through the internal durable `CoreDb` implementation while broad durable HTTP serving was still fail-closed.
...
## Hosted Storage Operations Hardening

This slice hardens hosted Postgres and R2/S3-compatible storage posture while keeping durable-cloud behind explicit readiness and storage posture gates.
...
What was not built in that hosted-storage slice:

- No production broad durable-cloud rollout, encrypted/KMS-backed secret replay, destructive final-object deletion, broad unreachable commit/object deletion, distributed locks, or event-bus audit pipeline.
- No durable-cloud mutation serving outside guarded mounted-session routes.
...
- `ObjectCleanupDeletionMode` remains non-destructive by default. The HTTP recovery route reports `deletion_enabled: false`, and the durable recovery scheduler instantiates non-destructive cleanup without enabling byte deletion.
- A first eligible cleanup pass persists `deletion_ready_at`, `delete_after`, and a redacted metadata snapshot before any destructive action. Ready rows can be reported as held, deferred, poisoned, remaining, or deleted without exposing canonical object keys, lease tokens, raw backend errors, DB URLs, endpoints, commit messages, or request bodies.
- The destructive worker path applies only to cleanup-claim-owned `DurableMutationCasLostObjectCleanup` final objects. It requires an active cleanup claim, expired hold window, current persisted readiness, matching metadata snapshot, active final-object metadata fence, repeated reachability proof, and immediate claim/fence revalidation before byte deletion.
- Byte deletion and metadata deletion are recorded as separate phase markers. A cleanup claim can complete only after both final-object bytes and fenced metadata deletion are recorded, making crashes between phases retry-visible.
- In-memory and Postgres cleanup claim stores persist deletion readiness plus byte/metadata phase markers. Postgres migration `0012_object_cleanup_deletion_state.sql` adds the readiness/snapshot/phase columns and constraints CAS-lost cleanup completion on both phase markers.
- `BlobObjectStore` and the local memory object store delete final bytes only when the caller supplies the canonical expected final-object key for the repo/object/kind.
- Broad unreachable commit/object GC remains a dry-run/status protocol. No general commit metadata or object record deletion is executed by this slice.
...
Grounding: `docs/plans/2026-05-15-destructive-final-object-deletion-unreachable-gc-protocol.md`, `src/backend/object_cleanup.rs`, `src/backend/blob_object.rs`, `src/backend/mod.rs`, `src/backend/postgres.rs`, `migrations/postgres/0012_object_cleanup_deletion_state.sql`, `src/server/routes_vcs.rs`, `src/server/mod.rs`, `docs/http-api-guide.md`.

## Bounded Guarded Commit Repair Worker

The guarded durable commit path can persist route-bound post-CAS repair context and run a bounded, admin-triggered repair worker for visible commits. At this slice's landing time, broad durable core routing still remained fail-closed while workspace-head, audit, and idempotency completion gaps became repairable without guessing from commit IDs alone.
...
- At this slice's landing time, there was no broad `STRATUM_CORE_RUNTIME=durable-cloud` server startup; it remained fail-closed before auth or routes were available.
...
- At this slice's landing time, there was no broad `STRATUM_CORE_RUNTIME=durable-cloud` route enablement beyond the guarded durable capability path.
...
- The plan kept broad durable startup fail-closed until durable auth/session routing, policy below route layer, explicit tenant/repo routing, hosted storage posture, idempotency retention/quota, recovery readiness, and non-HTTP bypass handling were solved by later implementation slices.
...
## Tenant/Repo Routing Foundation

This slice replaces hosted durable `RepoId::local()` assumptions at HTTP route seams with explicit request repo context while preserving local singleton compatibility. It does not enable broad `STRATUM_CORE_RUNTIME=durable-cloud`.

What is built:

- `RequestRepoContext` resolves repo identity from mounted workspace sessions or the admin `X-Stratum-Repo` selector and rejects invalid or mismatched identities.
- Local singleton paths still intentionally default to `RepoId::local()`/legacy local idempotency scopes where no repo context is present.
- Non-local guarded durable paths fail closed when a workspace bearer session or admin durable request lacks explicit repo identity.
- Guarded durable FS/VCS/review route plumbing uses request-scoped durable capabilities instead of a hard-coded local repo where this slice touches the seam.
- Workspace creation can persist explicit repo identity through in-memory, local-file, and Postgres metadata stores; legacy local workspace creation remains `repo_id: None`.
- Workspace bearer validation rejects workspace/token repo mismatches and cannot turn a missing hosted durable repo into a global/local fallback session.
- Review protected ref rules, protected path rules, change requests, approvals, reviewer assignments, comments, approval decisions, and route policy evaluation are repo-scoped.
- Review merge loads the change request through the routed repo and uses that repo for durable ref/commit reads and target-ref CAS, so a change request cannot advance another repo's target ref through route selection.
- FS, VCS, review, and workspace idempotency scopes are repo-qualified for explicit repo contexts while preserving legacy local singleton replay behavior.
- Postgres review and workspace adapters now read/write existing `repo_id` columns from the caller context instead of hard-coding `RepoId::local()`.
- Durable post-CAS workspace completion/recovery uses the routed repo for workspace head updates, and VCS commit idempotency replay bodies redact commit messages.
...
Grounding: `docs/plans/2026-05-12-tenant-repo-routing-foundation.md`, `src/server/repo_context.rs`, `src/auth/session.rs`, `src/workspace/mod.rs`, `src/review.rs`, `src/backend/postgres.rs`, `src/backend/core_transaction.rs`, `src/server/middleware.rs`, `src/server/policy.rs`, `src/server/routes_workspace.rs`, `src/server/routes_fs.rs`, `src/server/routes_vcs.rs`, `src/server/routes_review.rs`.

## Broad Durable Core Runtime Incremental Enablement

This slice originally added a tightly gated `STRATUM_CORE_RUNTIME=durable-cloud` HTTP server path for read-only durable core execution. It was an incremental enablement step, not a production hosted rollout; later slices expanded the durable-cloud route set and removed the `STRATUM_DURABLE_CORE_RUNTIME_ENABLE_DEV` startup requirement while retaining the other readiness/storage gates.

What is built:

- At original landing, durable-cloud startup required `STRATUM_BACKEND=durable`, `STRATUM_DURABLE_CORE_RUNTIME_ENABLE_DEV=1`, durable auth/session, policy, repo-routing, and recovery readiness gates, plus a non-local `STRATUM_DURABLE_CORE_REPO_ID`. Current durable-cloud startup no longer requires `STRATUM_DURABLE_CORE_RUNTIME_ENABLE_DEV`; it still requires the durable backend, readiness gates, non-local repo id, idempotency retention/quota config, explicit Postgres posture, and explicit R2 posture.
- Durable-cloud rejects `STRATUM_DURABLE_COMMIT_ROUTE=1`; the guarded durable route remains a separate local-state capability path.
- When durable-cloud gates pass, `stratum-server` opens durable stores and constructs `DurableCoreRuntime` directly from `StratumStores` and the configured repo id.
- Durable-cloud startup does not open or create local `.vfs/state.bin`; `/health` reports `core_runtime: durable-cloud` with local core counters as `null`.
- At original landing, the durable-cloud router exposed only `GET /fs`, `GET /fs/{path}`, `GET /search/grep`, `GET /search/find`, `GET /tree`, `GET /tree/{path}`, `GET /vcs/log`, `GET /vcs/status`, `GET /vcs/diff`, and `GET /vcs/refs`; later slices added mounted-session FS mutations, VCS mutations, and review/protected-change routes.
- Durable-cloud FS/search/tree reads use committed durable object, commit, and ref stores. Durable VCS reads and mutations use durable commit/ref metadata and status/diff over durable main or mounted session refs.
- Durable-cloud workspace bearer auth requires an explicit mounted repo, a durable principal, and a repo match with the router repo. Conflicting, duplicate, malformed, or missing repo context fails closed with redacted errors and no `RepoId::local()` fallback.
- Unsupported route groups, including auth login, workspaces, runs, audit listing, semantic search, execution, and VCS recovery operator routes, return the stable durable-cloud unsupported JSON `501`; mounted-session FS mutations, VCS mutations, and review/protected-change routes have since been enabled under the durable-cloud route set.
- Existing local-state behavior remains unchanged, including `STRATUM_DURABLE_COMMIT_ROUTE=1` guarded committed reads, mounted-session durable FS mutations, VCS metadata/status/diff/revert, and recovery routes.

What remains fail-closed or out of scope:

- No production hosted rollout; the durable-cloud path remains behind explicit readiness, repo, idempotency, Postgres posture, and R2 posture gates.
...
Grounding: `docs/plans/2026-05-12-broad-durable-core-runtime-incremental-enablement.md`, `src/backend/runtime.rs`, `src/bin/stratum_server.rs`, `src/server/mod.rs`, `src/server/core.rs`, `src/server/middleware.rs`, `src/server/repo_context.rs`, `src/server/routes_auth.rs`, `src/server/routes_fs.rs`, `src/server/routes_vcs.rs`, `tests/server_startup.rs`.

## Durable-Cloud Mounted-Session FS Mutations

This slice moved the existing guarded durable mounted-session filesystem mutation path into the then-dev-gated `STRATUM_CORE_RUNTIME=durable-cloud` router without enabling broader durable-cloud mutation groups.

What is built:

- Durable-cloud now mounts `PUT /fs/{path}`, `PATCH /fs/{path}`, `DELETE /fs/{path}`, and `POST /fs/{path}?op=copy|move` for workspace bearer sessions that carry a durable `session_ref`.
- Durable-cloud FS mutations reuse the existing durable mutation executor: the route evaluates policy, converts an allowed decision into a policy token, applies a durable mutation commit, and advances only the workspace session ref through existing ref CAS semantics.
- Durable-cloud mutable-session preflights use durable committed/session-ref state for write, mkdir, metadata, delete, copy, move, replay, destination, and directory checks before idempotency reservation or mutation execution.
- Route idempotency, policy audit, durable FS mutation audit identity, and durable FS mutation recovery claims now work through a filesystem-only durable mutation capability seam. The seam exposes durable FS mutation stores and output methods but does not expose guarded VCS/recovery capabilities from `DurableCoreRuntime`.
- Missing workspace bearer auth, missing `session_ref`, cross-repo workspace identity, conflicting repo headers, and write-scope denial fail closed without materializing session refs or mutating durable objects.
- Durable-cloud capability manifests now advertise filesystem write/patch/delete/copy/move as available and idempotent with `requires: ["workspace-bearer", "durable-session-ref"]`. Durable-cloud idempotency support is advertised only for the FS mutation endpoints.
- Durable-cloud startup still rejects `STRATUM_DURABLE_COMMIT_ROUTE=1`, opens no local `.vfs/state.bin`, and does not require the guarded local-state route gate.

What remains fail-closed or out of scope:

- At this slice's landing time, VCS commit/revert/ref create/ref update/recovery, review/protected-change routes, workspace management, auth login, audit reads, run records, MCP, CLI direct local execution, FUSE, production hosted rollout, and destructive cleanup controls remained unsupported under durable-cloud. Later slices enabled VCS commit/revert/ref create/ref update and review/protected-change routes; recovery operator routes, workspace management, auth login, audit reads, run records, MCP, CLI direct local execution, FUSE, production hosted rollout, and destructive cleanup controls remain unsupported.
- Non-token durable FS mutation methods still fail closed with policy-token-required semantics.
- This slice does not add distributed locks, connection-pool changes, new CAS semantics, general durable workspace mutation APIs outside mounted-session FS routes, or live infra provisioning.

Verification on 2026-05-16 from the `v2/foundation` worktree: spec/correctness review reported no findings. Code-quality/security review found durable FS mutation CAS/status flattening, weak capability copy/move probes, and an unbumped capability revision; main-session fixes mapped durable FS CAS conflicts to `409`, backend/store failures to `500`, tightened the concurrency test, switched probes and the plan doc to `dst`, bumped capability revision to `2026-05-16-1`, and regenerated SDK fixtures. Focused gates passed for `cargo test --locked server::routes_fs::tests::durable_cloud --lib -- --nocapture` (**7** tests), `cargo test --locked server::routes_fs::tests::guarded_durable --lib -- --nocapture` (**18** tests), `cargo test --locked server::tests::durable_core_router_returns_stable_501_for_unsupported_groups --lib -- --nocapture` (**1** test), `cargo test --locked server::routes_capabilities::tests::durable_cloud --lib -- --nocapture` (**2** tests), `STRATUM_UPDATE_CAPABILITY_FIXTURES=1 cargo test --locked server::routes_capabilities::tests::update_checked_in_sdk_contract_fixture_when_requested --lib -- --nocapture`, `cargo test --locked server::routes_capabilities::tests::update_checked_in_sdk_contract_fixture_when_requested --lib -- --nocapture`, and `cargo test --locked --test server_startup durable -- --nocapture` (**15** tests). Full gates passed: `cargo fmt --all -- --check`; `git diff --check`; `STRATUM_POSTGRES_TEST_URL= ./scripts/check-postgres-migrations.sh` skipped cleanly because the URL was unset; `STRATUM_R2_TEST_ENABLED= ./scripts/check-r2-object-store.sh` skipped cleanly because R2 was not enabled; `cargo test --locked --features postgres backend::postgres --lib -- --nocapture` passed **28** tests with live Postgres portions skipped because `STRATUM_POSTGRES_TEST_URL` was unset; `cargo clippy --locked --all-targets -- -D warnings`; `cargo clippy --locked --all-targets --features postgres -- -D warnings`; `cargo test --locked --lib --tests`, including **842** lib tests, **9** `stratum_mcp` unit tests, **5** `stratumctl` unit tests, **142** integration tests, **37** perf tests, **1** perf-comparison test, **72** permissions tests, and **19** server-startup tests; and `cargo audit --deny warnings` after scanning **409** crate dependencies. Live Postgres/R2 credentials were not present, so the exact live CI gates were not run against real hosted services.

Grounding: `docs/plans/2026-05-16-durable-cloud-mounted-session-fs-mutations.md`, `src/server/core.rs`, `src/server/routes_fs.rs`, `src/server/mod.rs`, `src/server/routes_capabilities.rs`, `sdk/contracts/capabilities.v1.durable-cloud.json`, `docs/http-api-guide.md`, `tests/server_startup.rs`.

## MCP/CLI/FUSE/Embedded Durable Auth, Policy, And Repo-Routing Parity

This slice closes the immediate non-HTTP bypass risk after the initial durable-cloud read router landed.

What is built:

- `stratumctl` remains a thin HTTP client and now accepts `--repo` / `STRATUM_REPO`, sending `X-Stratum-Repo` on every request when configured.
- `StratumClient` sends auth, workspace, and repo headers consistently for JSON routes, text routes, read commands, and mutation commands. Stratum-invalid repo ids fail locally with a redacted `invalid repo header` error before a request is sent.
- Durable-cloud unsupported mutations still surface the stable redacted `501` JSON body through the client.
- A shared non-server runtime guard rejects `STRATUM_CORE_RUNTIME=durable-cloud` and its aliases for direct local binaries before durable backend env parsing and before local `StratumDb` open.
- `stratum-mcp`, `stratum-mount`, and the local REPL call that guard before opening local state. The REPL no longer falls back to in-memory local state when durable-cloud is selected.
- The guard rejects non-UTF8 `STRATUM_CORE_RUNTIME` values with a redacted invalid-runtime error.
- MCP remains a local-state stdio server; durable MCP writes and remote durable MCP reads remain future work.
- FUSE remains a local snapshot mount; durable-cloud FUSE fails closed until sparse mount/session/cache semantics land.
- Direct embedded `StratumDb` callers are documented as local-only and are not a hosted durable policy boundary.

What remains fail-closed or out of scope:

- No sparse remote FUSE.
- No MCP durable writes or remote HTTP MCP mode.
- No broad durable mutations, final-object deletion/GC, production hosted rollout, semantic search, web console, or execution runner.

Verification on 2026-05-13 from the `v2/foundation` worktree passed: `cargo fmt --all -- --check`; `git diff --check`; `cargo check --locked --features fuser --bin stratum-mount`; `cargo test --locked backend::runtime --lib -- --nocapture` (**41** tests); `cargo test --locked --bin stratum-mcp -- --nocapture` (**9** tests); `cargo test --locked --bin stratumctl -- --nocapture` (**5** tests); `cargo test --locked client --lib -- --nocapture` (**9** tests); `cargo test --locked --test server_startup durable_core_runtime -- --nocapture` (**5** tests); `cargo clippy --locked --all-targets -- -D warnings`; `cargo clippy --locked --all-targets --features postgres -- -D warnings`; `cargo test --locked --lib --tests`, including **730** lib tests, **9** `stratum_mcp` unit tests, **5** `stratumctl` unit tests, **142** integration tests, **37** debug perf tests, **1** debug perf-comparison test, **72** permission tests, and **12** server-startup tests; and `cargo audit --deny warnings` after scanning **408** crate dependencies. Final release perf used `sleep 10 && /usr/bin/time -l cargo test --locked --release --test perf -- --test-threads=1 --nocapture`; it passed **37** tests in **45.68s real**, **148.41s user**, **4.73s sys**, with **1,904,214,016 bytes max RSS** and **100,024,872 bytes peak memory footprint**. Spec/correctness review reported no findings. Code-quality/security review found partial workspace auth fallback, raw `ClientAuth` debug output, and MCP helper invalid-env handling issues; main-session fixes made partial workspace auth fail closed, redacted auth Debug output, marked authorization headers sensitive, and moved the MCP process helper onto the non-UTF8-aware runtime guard before this final verification sweep.
...
- `ObjectGcDryRun` walks durable roots from refs, workspace heads/session refs, retained review/change-request refs including terminal records, post-CAS recovery, pre-visibility recovery, durable FS mutation recovery, retained idempotency records including ref replay `target` fields, and active object cleanup claims.
- The scanner walks reachable commits to parents, root trees, tree children, and blobs, then reports bounded redacted unreachable commit/object candidates. Root-source failures suppress all deletion candidates; tree-walk blockers can still allow bounded unreachable commit reporting but suppress object candidates.
- `ObjectMetadataStore` now supports store-backed `FinalObjectMetadataFence` acquisition, validation, and release. Active fences block metadata repair/writes for the same repo/kind/id, and fence `Debug` output does not print canonical object keys or tokens.
- Postgres migration `0010_object_deletion_fences.sql` adds the fence table keyed by repo, object kind, and object id, and the Postgres adapter implements the same fence and repo-scoped cleanup-claim contracts as the in-memory store.
- `ObjectCleanupWorker` processes only due repo-scoped `DurableMutationCasLostObjectCleanup` claims. It acquires a cleanup claim, proves the object is unreachable, acquires a metadata fence, re-runs reachability with the current claim allowlisted, validates matching object metadata, revalidates the cleanup claim and fence, and reports `deletion_ready`.
- The worker is non-destructive: it releases the claim after readiness so the signal is repeatable, leaves bytes/metadata intact, does not complete the cleanup claim, keeps `deleted_final_objects` at `0`, records retry/backoff failures with fixed redacted diagnostics, blocks metadata-missing final objects as repairable, and sorts max-attempt rows behind claimable work so poisoned rows do not starve the bounded worker.
- `GET /vcs/recovery` exposes a bounded `gc_dry_run` summary plus cleanup-claim visibility without canonical object keys, raw object-store errors, idempotency keys, lease tokens, or commit messages.
- `POST /vcs/recovery/run` and the durable recovery scheduler include object cleanup as a fourth bounded phase after pre-visibility, post-CAS, and FS mutation recovery. Route summaries include `deletion_enabled: false`, `deleted_final_objects`, `deletion_ready`, per-skip counters, retry/backoff, poison, deferred, and repo-scoped remaining counts.
...
Grounding: `docs/plans/2026-05-13-final-object-deletion-unreachable-object-gc.md`, `src/backend/object_cleanup.rs`, `src/backend/blob_object.rs`, `src/backend/core_transaction.rs`, `src/backend/postgres.rs`, `src/server/routes_vcs.rs`, `src/server/mod.rs`, `migrations/postgres/0010_object_deletion_fences.sql`.
...
- At this slice's landing time, there was no broad `STRATUM_CORE_RUNTIME=durable-cloud` route enablement; the later Broad Durable Core Runtime Incremental Enablement slice added the initial gated read-only router.
...
The guarded durable commit route persists post-CAS recovery work for visible commits before it returns or replays a normal redacted partial response. At this slice's landing time, broad durable runtime remained fail-closed while visible post-CAS completion gaps became inspectable and safely claimable across process restarts.
