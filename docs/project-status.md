...
- Unsupported route groups, including auth login, runs, audit listing, semantic search, execution, and VCS recovery operator routes, return the stable durable-cloud unsupported JSON `501`; mounted-session FS mutations, VCS mutations, review/protected-change routes, and workspace management routes have since been enabled under the durable-cloud route set.
