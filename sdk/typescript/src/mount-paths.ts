import type { StratumDirectoryEntry, StratumDirectoryListing, StratumStat } from "./types.js";

export interface IndexedPathEntry extends StratumDirectoryEntry {
  readonly path: string;
}

export function normalizeMountPath(input: string, cwd = "/"): string {
...
export const normalizePath = normalizeMountPath;

export function toClientPath(input: string, cwd = "/"): string {
  return normalizeMountPath(input, cwd).replace(/^\/+/, "");
}
...
export class PathIndex {
  ...
  recordListing(listing: StratumDirectoryListing, cwd = "/"): void {
    const directoryPath = normalizeMountPath(listing.path, cwd);
    for (const entry of listing.entries) {
      assertDirectoryEntryName(entry.name, directoryPath);
    }

    this.entries.set(directoryPath, directoryEntry(directoryPath));

    for (const entry of listing.entries) {
      const childPath = normalizeMountPath(entry.name, directoryPath);
  ...
  entry(path: string): IndexedPathEntry | undefined {
    return this.entries.get(normalizeMountPath(path));
  }
...
function assertDirectoryEntryName(name: string, parent: string): void {
  if (name === "" || name === "." || name === ".." || name.includes("/")) {
    throw new Error(`Invalid directory entry name '${name}' in listing '${parent}'`);
  }
}
