import type { StratumDirectoryListing, StratumStat } from "./types.js";
...
export class SessionCache {
  ...
  getRead(path: string): CachedRead | null {
    return cloneRead(this.get<CachedRead>("read", path));
  }

  setRead(path: string, content: CachedRead): void {
    this.set("read", path, cloneRead(content), byteLength(content));
  }

  getStat(path: string): StratumStat | null {
    const stat = this.get<StratumStat>("stat", path);
    return stat === null ? null : cloneStat(stat);
  }

  setStat(path: string, stat: StratumStat): void {
    this.set("stat", path, cloneStat(stat), byteLength(JSON.stringify(stat)));
  }

  getList(path: string): StratumDirectoryListing | null {
    const listing = this.get<StratumDirectoryListing>("list", path);
    return listing === null ? null : cloneListing(listing);
  }

  setList(path: string, listing: StratumDirectoryListing): void {
    this.set("list", path, cloneListing(listing), byteLength(JSON.stringify(listing)));
  }
...
function cacheKey(kind: SessionCacheKind, path: string): string {
  return `${kind}:${normalizeMountPath(path)}`;
}
...
function cloneRead<T extends CachedRead | null>(content: T): T {
  if (content instanceof Uint8Array) return new Uint8Array(content) as T;
  return content;
}

function cloneStat(stat: StratumStat): StratumStat {
  return {
    ...stat,
    custom_attrs: { ...stat.custom_attrs },
  };
}

function cloneListing(listing: StratumDirectoryListing): StratumDirectoryListing {
  return {
    path: listing.path,
