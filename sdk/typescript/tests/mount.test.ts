...
describe("StratumVolume", () => {
  ...
  it("caches directory listings and file reads by normalized path", async () => {
    ...
    await expect(volume.listDirectory("docs")).resolves.toEqual(listing("/docs"));
    await expect(volume.cat("/docs/README")).resolves.toBe("hello");
    await expect(volume.readFile("docs/README")).resolves.toBe("hello");
  ...
  it("preserves binary reads and writes through the volume cache", async () => {
    ...
    const first = await volume.readFileBuffer("/bin/data");
    first[0] = 0x00;

    await expect(volume.readFileBuffer("/bin/data")).resolves.toEqual(new Uint8Array([0xff, 0x00, 0x61]));
    ...
    expect(client.writeFile).toHaveBeenCalledWith("bin/out", new Uint8Array([0xff, 0x00, 0x61]), undefined);
  });

  it("snapshots binary write content before awaiting the client write", async () => {
    const bytes = new Uint8Array([0xff, 0x00, 0x61]);
    const client = createClient();
    let resolveWrite: (() => void) | undefined;
    let capturedContent: StratumRequestBody | undefined;
    client.writeFile.mockImplementation(async (_path, content) => {
      capturedContent = content;
      await new Promise<void>((resolve) => {
        resolveWrite = resolve;
      });
      return { written: "/bin/out", size: 3 };
    });
    const volume = new StratumVolume(client);

    const write = volume.writeFile("/bin/out", bytes);
    bytes[0] = 0x00;

    expect(capturedContent).toEqual(new Uint8Array([0xff, 0x00, 0x61]));
    resolveWrite?.();
    await write;
    await expect(volume.readFileBuffer("/bin/out")).resolves.toEqual(new Uint8Array([0xff, 0x00, 0x61]));
  });

  it("rejects root file and destructive operations at the mount boundary", async () => {
    const client = createClient();
    const volume = new StratumVolume(client);

    await expect(volume.readFile("/")).rejects.toThrow("is a directory");
    await expect(volume.readFileBuffer("/")).rejects.toThrow("is a directory");
    await expect(volume.writeFile("/", "content")).rejects.toThrow("is a directory");
    await expect(volume.mkdir("/")).rejects.toThrow("operation not permitted");
    await expect(volume.deletePath("/")).rejects.toThrow("operation not permitted");
    await expect(volume.copyPath("/", "/copy")).rejects.toThrow("operation not permitted");
    await expect(volume.copyPath("/docs/a.txt", "/")).rejects.toThrow("operation not permitted");
    await expect(volume.movePath("/", "/copy")).rejects.toThrow("operation not permitted");
    await expect(volume.movePath("/docs/a.txt", "/")).rejects.toThrow("operation not permitted");

    expect(client.readFile).not.toHaveBeenCalled();
    expect(client.readFileBuffer).not.toHaveBeenCalled();
    expect(client.writeFile).not.toHaveBeenCalled();
    expect(client.mkdir).not.toHaveBeenCalled();
    expect(client.deletePath).not.toHaveBeenCalled();
    expect(client.copyPath).not.toHaveBeenCalled();
    expect(client.movePath).not.toHaveBeenCalled();
  });
  ...
  it("delegates mutations with client paths and invalidates affected cached paths", async () => {
    const client = createClient();
    client.readFile.mockResolvedValue("old");
    ...
    await volume.copyPath("/docs/a.txt", "/docs/b.txt", { idempotencyKey: "copy-1" });
    await volume.movePath("/docs/b.txt", "/archive/b.txt", { idempotencyKey: "move-1" });
    await volume.deletePath("/archive/b.txt", false, { idempotencyKey: "rm-1" });
    await expect(volume.cat("/docs/a.txt")).resolves.toBe("old");
  ...
  it("delegates search, tree, and VCS methods using cwd-aware paths", async () => {
    ...
    const volume = new StratumMount(client, { cwd: "/workspace" });
  ...
  it("mounts from StratumClient and delegates through current auth and route behavior", async () => {
    const requests: Request[] = [];
    const fetchImpl: typeof fetch = async (input, init) => {
      requests.push(new Request(input, init));
      return Response.json(listing("/docs"));
    };
    ...
    const volume = client.mount({ cwd: "/workspace" });
    expect(volume).toBeInstanceOf(StratumVolume);
    await volume.ls("docs");

    expect(requests[0]?.url).toBe("https://stratum.example/api/fs/workspace/docs");
