...
import type { StratumDirectoryListing, StratumStat } from "../src/index.js";
import { SessionCache } from "../src/index.js";
...
describe("SessionCache", () => {
  ...
  it("stores binary read entries defensively without converting them to strings", () => {
    ...
    bytes[0] = 0x00;
    const cached = cache.getRead("/bin/data");
    if (!(cached instanceof Uint8Array)) throw new Error("expected binary cache entry");
    cached[1] = 0xff;

    expect(cache.getRead("/bin/data")).toEqual(new Uint8Array([0xff, 0x00, 0x61]));
  ...
  it("defensively copies cached stat and list objects", () => {
    const cache = new SessionCache({ ttlMs: null });
    const mutableStat = { ...stat, custom_attrs: { team: "alpha" } };
    const mutableListing: StratumDirectoryListing = {
      path: "/docs",
      entries: [{ ...listing.entries[0]!, name: "README" }],
    };

    cache.setStat("/docs/README", mutableStat);
    cache.setList("/docs", mutableListing);
    mutableStat.custom_attrs.team = "mutated";
    (mutableListing.entries[0] as unknown as { name: string }).name = "mutated";

    const cachedStat = cache.getStat("/docs/README");
    const cachedListing = cache.getList("/docs");
    cachedStat!.custom_attrs.team = "reader-mutated";
    (cachedListing!.entries[0] as unknown as { name: string }).name = "reader-mutated";

    expect(cache.getStat("/docs/README")?.custom_attrs).toEqual({ team: "alpha" });
    expect(cache.getList("/docs")?.entries[0]?.name).toBe("README");
  });
